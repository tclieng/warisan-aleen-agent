...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Warisan Aleen · Dashboard')),
          ...
          builder: (c, snap) {
            ...
            if (snap.hasError) {
              return ListView(padding: const EdgeInsets.all(16), children: [
                ...
                const SizedBox(height: 8),
                const Text(
                  'Pull to refresh  ·  Change baseUrl in Settings',
                  textAlign: TextAlign.center,
            ...
            return ListView(padding: const EdgeInsets.all(16), children: [
              _card(
                'Backend',
                s['dry_run'] == true ? 'Dry Run (safe mode)' : 'Live Publishing',
                s['dry_run'] == true ? Colors.orange : Colors.green,
              ),
              _card(
                'Access Token',
                s['has_token'] == true ? 'Configured' : 'Not set (pending Meta approval)',
                s['has_token'] == true ? Colors.green : Colors.red,
              ),
              _card('Content Model', '${s['ollama_model'] ?? '-'}'),
              _card('Total Posts', '${s['post_count'] ?? 0}'),
              _card(
                'Status Breakdown',
